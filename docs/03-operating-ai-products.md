...
- [Human review for self-improving agents](#human-review-for-self-improving-agents)
...
## Human review for self-improving agents

Self-improvement is useful only when traces, evals, and human review constrain it. An agent can suggest improvements to prompts, evals, tools, routing, or workflows, but it should not silently change production behavior.
...
- Tool permission or tool instruction changes.
- Workflow changes that alter who reviews, approves, or can roll back.
- Any change generated by an automated improvement loop.
...
The bigger the blast radius, the stricter the review. Internal PM agents can move faster. User-facing agents need launch gates, rollback, audit trails, and staged rollout.
